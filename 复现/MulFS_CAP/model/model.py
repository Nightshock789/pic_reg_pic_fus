...
class ImageTransform(nn.Module):
    ...
    def generate_grid(self, input):
        device = input.device
        batch_size, _, height, weight = input.size()  # 动态获取尺寸
...
def window_partition(x, window_size, stride):
    ...
    if window_size is None:
        window_size = min(x.shape[-2], x.shape[-1]) // 4  # 示例：按1/4比例动态计算
    
    batch_size, channel, height, weight = x.size()
    unfold_win = nn.Unfold(kernel_size=(window_size, window_size), stride=stride)
    x_windows = unfold_win(x)  # [batch_size, p_w * p_w * channel, patch_nums]
    x_out_windows = x_windows.reshape(batch_size, channel, window_size, window_size, x_windows.size()[2]).permute(4, 0, 1, 2, 3)
    return x_out_windows
...
def df_window_partition(x, large_window_size, small_window_size, is_bewindow=True):
    ...
    """
    batch_size, channel, height, weight = x.size()
    if large_window_size is None:
        large_window_size = min(height, weight) // 4  # 示例：大窗口为1/4尺寸
    if small_window_size is None:
        small_window_size = large_window_size // 2   # 小窗口为大窗口的一半
